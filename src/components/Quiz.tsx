  ...
  return (
    <section id="quiz" className="py-20 px-6 max-w-4xl mx-auto">
      <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
        <h2 className="text-3xl font-bold mb-10 text-center">اختبر معلوماتك</h2>
        
        {showScore ? (
          <div className="text-center py-10">
            <div className="text-8xl mb-6">🏆</div>
            <h3 className="text-3xl font-extrabold mb-4">أحسنت يا بطل!</h3>
            <p className="text-xl text-slate-600 mb-8">لقد أجبت على {score} من أصل {QUIZ_QUESTIONS.length} بشكل صحيح.</p>
            <button
              onClick={restartQuiz}
              className="flex items-center gap-3 px-8 py-4 bg-brand-primary text-white rounded-2xl font-bold mx-auto hover:scale-105 transition-transform"
            >
              <RotateCcw size={20} />
              حاول مرة أخرى
            </button>
          ...
          <div>
            <div className="mb-8 overflow-hidden bg-slate-100 h-3 rounded-full flex">
              <div 
            ...
            <span className="text-slate-400 font-bold mb-2 block">السؤال {currentQuestion + 1} من {QUIZ_QUESTIONS.length}</span>
            <h3 className="text-2xl font-bold mb-10 text-slate-800">{QUIZ_QUESTIONS[currentQuestion].question}</h3>
            ...
            <div className="grid gap-4">
              {QUIZ_QUESTIONS[currentQuestion].options.map((option, index) => {
                ...
                let btnClass = "p-5 rounded-2xl border-2 text-right transition-all font-bold text-lg flex items-center justify-between ";
